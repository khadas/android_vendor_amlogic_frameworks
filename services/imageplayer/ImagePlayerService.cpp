...
SkBitmap* ImagePlayerService::decode(SkStreamRewindable *stream, InitParameter *mParameter) {
    ...
    if ((int)decodingBitmap->height()*decodingBitmap->rowBytes() < 4*decodingBitmap.width()* decodingBitmap.height()) {
        ALOGW("decode: bitmap size:%d, request size:%d\n",
            (int)decodingBitmap->height()*decodingBitmap->rowBytes(), 4*decodingBitmap.width()*decodingBitmap.height());
    }
...
bool ImagePlayerService::copyTo(SkBitmap* dst, SkColorType dstCT, const SkBitmap& src,
    ...
    if (checkCallingPermission(String16("android.permission.DUMP")) == false) {
        ...
        for (int i = 0; i + 1 < n; i++) {
            ...
            if (args[i] == option) {
                ...
                if (NULL != mBitmap) {
                    if ((int)mBitmap->height()*mBitmap->rowBytes() < 4*mBitmap->width()*mBitmap->height()) {
                        result.appendFormat("ImagePlayerService: [error]save origin bitmap RGBA data to file:%s, mBitmap size:%d, request size:%d\n",
                            path.string(), (int)mBitmap->height()*mBitmap->rowBytes(), 4*mBitmap->width()*mBitmap->height());
                    }
                    else {
                ...
                if (NULL != mBufBitmap) {
                    ...
                    if ((int)mBufBitmap->height()*mBufBitmap->rowBytes() < 4*mBufBitmap->width()*mBufBitmap->height()) {
                        result.appendFormat("ImagePlayerService: [error]save bitmap buffer RGBA data to file:%s, mBufBitmap size:%d, request size:%d\n",
                            bufPath, (int)mBufBitmap->height()*mBufBitmap->rowBytes(), 4*mBufBitmap->width()*mBufBitmap->height());
                    }
                    else {
                ...
                if (NULL != mRotateBitmap) {
                    ...
                    if ((int)mRotateBitmap->height()*mRotateBitmap->rowBytes() < 4*mRotateBitmap->width()*mRotateBitmap->height()) {
                        result.appendFormat("ImagePlayerService: [error]save rotate RGBA data to file:%s, mRotateBitmap size:%d, request size:%d\n",
                            bufPath, (int)mRotateBitmap->height()*mRotateBitmap->rowBytes(), 4*mRotateBitmap->width()*mRotateBitmap->height());
                    }
                    else {
                ...
                if (NULL != mScalingBitmap) {
                    ...
                    if ((int)mScalingBitmap->height()*mScalingBitmap->rowBytes() < 4*mScalingBitmap->width()*mScalingBitmap->height()) {
                        result.appendFormat("ImagePlayerService: [error]save scale RGBA data to file:%s, mScalingBitmap size:%d, request size:%d\n",
                            bufPath, (int)mScalingBitmap->height()*mScalingBitmap->rowBytes(), 4*mScalingBitmap->width()*mScalingBitmap->height());
                    }
                    else {
