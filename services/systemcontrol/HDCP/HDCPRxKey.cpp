...
#define HDCP_RX22_CFG_AIC_SRC               "/system/etc/firmware/hdcp_rx22/firmware.aic"
#define HDCP_RX22_CFG_AIC_DES               "/param/firmware.aic"
#define HDCP_RX22_KEY_PATH                  "/param/hdcp2_rx.bin"

#define HDCP_RX22_OUT_KEY_IMG               "/param/dcp_rx.out"
#define HDCP_RX22_OUT_KEY_LE                "/param/hdcp_keys.le"
#define HDCP_RX22_OUT_2080_BYTE             "/param/2080.byte"

#define HDCP_RX22_SRC_FW_PATH               "/system/etc/firmware/hdcp_rx22/firmware.le"
...
#else
    keyLen = readSys(HDCP_RX_KEY_ENCRYPT_TEST, pKeyBuf, HDCP_RX22_STORAGE_KEY_SIZE);
    if (keyLen < HDCP_RX22_KEY_CRC_LEN) {
        SYS_LOGE("read key length fail, at least %d bytes, but read len = %d\n", HDCP_RX22_KEY_CRC_LEN, keyLen);
        goto _exit;
    }
...
bool HDCPRxKey::combineFirmware() {
    bool ret = false;
    int srcFd = -1;
    int desFd = -1;
    int insertFd = -1;
    ...
    insertFd = open(HDCP_RX22_OUT_2080_BYTE, O_RDONLY);
    ...
    if ((desFd = open(HDCP_RX22_DES_FW_PATH, O_CREAT | O_RDWR | O_TRUNC, 0644)) < 0) {
        SYS_LOGE("combine firware, open %s error(%s)", HDCP_RX22_DES_FW_PATH, strerror(errno));
        goto exit;
    }
