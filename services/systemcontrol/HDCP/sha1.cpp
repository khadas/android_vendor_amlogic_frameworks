...
int gen_sha1sum_verify(const char* srFile, char* verifyData)
{
    ...
    int64_t SrcFileSz = 0;
    sha1_context ctx;
    int64_t totalReadSz = 0;
    unsigned thisReadSz = 0;
    struct stat64 s;
    int i = 1;
    ...
    {
        unsigned readSz = 0;
        int64_t leftSz = SrcFileSz - totalReadSz;
...
int append_sha1sum_to_buf(const char* pdat, unsigned datSz)
{
    ...
    sha1_context ctx;
    sha1_starts(&ctx);
    sha1_update(&ctx, (unsigned char*)pdat, datSz);
    sha1_finish(&ctx, (unsigned char*)pdat + datSz);
